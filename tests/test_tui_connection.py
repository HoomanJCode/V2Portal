...
from v2portal.tui import app_screen, connection_screen
...
def test_tui_connect_dispatches_selected_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        app_screen.widgets,
        "pick_profile",
        lambda profiles, groups, subscriptions=(), servers=(): ("profile", profile.id),
    )
    monkeypatch.setattr(
        app_screen,
        "run_connection",
...
def test_connection_screen_switches_target(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "pick_profile",
        lambda profiles, groups, subscriptions=(), servers=(): ("profile", second.id),
    )
    rendered = []
    monkeypatch.setattr(
        connection_screen,
